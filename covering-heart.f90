...
program algencanma
  ...
  ! PARAMETERS
  integer, parameter :: ndiff = 4, ntrials = 100
  integer, parameter :: diffnballs(ndiff) = (/ 3, 7, 11, 15 /)
...
10         continue
           ...
           inA = ( p(1) ** 2 + p(2) ** 2 - 1.0d0 ) ** 3 - p(1) ** 2 * p(2) ** 3 .le. 0.0d0
           if ( .not. inA ) then
              go to 10
           end if
           x(2*i-1) = p(1)
...
subroutine drawsol(n,x)
  ...
  ! LOCAL SCALARS
  integer :: i,j,k,npx,npy
  real(kind=8) :: bestfit,fit,px,pxbest,pxini,pxend,py,pyini,pyend,scale

  ! LOCAL ARRAYS
  character(len=80) :: filename

  write(filename,"(A15,I0,A3)") 'covering-heart-',nballs,'.mp'
  
  open(unit=10,file=trim(filename))

  ! BEGINNING
  scale = 4.85d0 / ( max( dtr(1) - dbl(1), dtr(2) - dbl(2) ) )
  write(10,10) nballs,scale
  ...
  ! Left side, y from -1 to 0.99
  npy = 200
  pyini = - 1.00d0
  pyend =   0.99d0
  do j = 1,npy
     py = pyini + ( j - 1 ) * ( pyend - pyini ) / ( npy - 1 )
     pxini = -1.5d0
     pxend =  0.0d0
     bestfit = 1.0d+20
     do i = 1,npx
        px = pxini + ( i - 1 ) * ( pxend - pxini ) / ( npx - 1 )
        fit = abs( ( px ** 2 + py ** 2 - 1.0d0 ) ** 3 - px ** 2 * py ** 3 )
        if ( fit .lt. bestfit ) then
           pxbest = px
           bestfit = fit
        end if
     end do
     write(10,21) pxbest,py
  end do

  ! Left side, y from 1.01 to 1.5
  npy = 50
  pyini = 1.01d0
  pyend = 1.50d0
  do j = 1,npy
     ...
     if ( bestfit .le. 1.0d-02 ) then
        write(10,21)  pxbest,py
     end if
  end do

  ! Left side, y from 1.5 to 1.0 (negative step)
  npy = 50
  pyini = 1.0d0
  pyend = 1.5d0
  do j = 1,npy
     py = pyend - ( j - 1 ) * ( pyend - pyini ) / ( npy - 1 )
     pxini = -0.5d0
     pxend =  0.0d0
     bestfit = 1.0d+20
     do i = 1,npx
  ...
  ! Right side, y from 1.5 to 1.01 (negative step)
  npy = 50
  pyini = 1.01d0
  pyend = 1.50d0
  do j = 1,npy
  ...
  ! Right side, y from 0.99 to -1 (negative step)
  npy = 200
  pyini = - 1.00d0
  pyend =   0.99d0
  do j = 1,npy
...
10 format('prologues := 3;',/, &
          ...
          'beginfig(',i3,');',/, &
          'u = ',f20.10,' cm;')
20 format('path heart;',/, &
          'heart := ');
21 format('     (',f20.10,'u,',f20.10,'u)--')
22 format('     cycle;',/, &
          'fill heart withcolor 0.9Dandelion;',/, &
          'draw heart withpen pencircle scaled 0.7;')
