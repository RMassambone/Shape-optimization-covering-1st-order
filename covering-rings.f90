...
program algencanma
  ...
  integer, parameter :: diffnballs(ndiff) = (/ 3, 7, 11 /)

  ! COMMON SCALARS
  integer :: nballs,atype
  real(kind=8) :: h
  ...
  ! COMMON BLOCKS
  common /pdata/ dbl,dtr,h,nballs,atype
...
1 continue
  ...
  vparam(2) = 'LARGEST-PENALTY-PARAMETER-ALLOWED 1.0d+06'
...
10         continue
           ...
           if ( atype .eq. 1 ) then
              inA = p(1) ** 2 + p(2) ** 2 .le. 0.5d0 ** 2 .and. p(1) ** 2 + p(2) ** 2 .ge. 0.35d0 ** 2
           else if ( atype .eq. 2 ) then
              inA = p(1) .le. 0.0d0 .and. p(1) ** 2 + p(2) ** 2 .le. 0.5d0 ** 2 .and. p(1) ** 2 + p(2) ** 2 .ge. 0.35d0 ** 2
           else
              inA = ( p(1) .le. 0.0d0 .and. &
                      p(1) ** 2 + ( p(2) - 0.175d0 ) ** 2 .le. 0.25d0 ** 2 .and. &
                      p(1) ** 2 + ( p(2) - 0.175d0 ) ** 2 .ge. 0.10d0 ** 2 ) .or. &
                    ( p(1) .ge. 0.0d0 .and. &
                      p(1) ** 2 + ( p(2) + 0.175d0 ) ** 2 .le. 0.25d0 ** 2 .and. &
                      p(1) ** 2 + ( p(2) + 0.175d0 ) ** 2 .ge. 0.10d0 ** 2 )
           end if
           if ( .not. inA ) then
              go to 10
           end if
  ...
  do i = 1,nx
     ...
     do j = 1,ny
        ...
        if ( atype .eq. 1 ) then
           ...
           inA = ( p(1) .le. 0.0d0 .and. &
                   p(1) ** 2 + ( p(2) - 0.175d0 ) ** 2 .le. 0.25d0 ** 2 .and. &
                   p(1) ** 2 + ( p(2) - 0.175d0 ) ** 2 .ge. 0.1d0 ** 2 ) .or. &
                 ( p(1) .ge. 0.0d0 .and. &
                   p(1) ** 2 + ( p(2) + 0.175d0 ) ** 2 .le. 0.25d0 ** 2 .and. &
                   p(1) ** 2 + ( p(2) + 0.175d0 ) ** 2 .ge. 0.1d0 ** 2 )
        end if
...
subroutine drawsol(n,x)
  ...
  if ( atype .eq. 1 ) then
     write(filename,"(A14,I0,A3)") 'covering-ring-',nballs,'.mp'
  else if ( atype .eq. 2 ) then
     write(filename,"(A15,I0,A3)") 'covering-ringC-',nballs,'.mp'
  else
     write(filename,"(A15,I0,A3)") 'covering-ringS-',nballs,'.mp'
  end if
...
10 format('prologues := 3;',/, &
          ...
          'u = 4.0 cm;')
11 format('path A;',/, &
          'A = (0u,0.5u){left}..{down}(-0.5u,0u){down}..{right}',/, &
          '    (0u,-0.5u)--(0u,-0.35u){left}..{up}(-0.35u,0u){up}..{right}',/, &
          '    (0u,0.35u)--cycle;',/, &
          'fill A withcolor 0.9Dandelion;',/, &
          'fill A rotated 180 withcolor 0.9Dandelion;',/, &
          'draw fullcircle scaled 1.0u shifted (0u,0u) withpen pencircle scaled 0.7;',/, &
          'draw fullcircle scaled 0.7u shifted (0u,0u) withpen pencircle scaled 0.7;')
12 format('path A;',/, &
          'A = (0u,0.5u){left}..{down}(-0.5u,0u){down}..{right}',/, &
          '    (0u,-0.5u)--(0u,-0.35u){left}..{up}(-0.35u,0u){up}..{right}',/, &
          '    (0u,0.35u)--cycle;',/, &
          'fill A withcolor 0.9Dandelion;',/, &
          'draw A withpen pencircle scaled 0.7;')
13 format('path A;',/, &
          'A = (0u,0.425u){left}..{down}(-0.25u,0.175u){down}..{right}',/, &
          '    (0u,-0.075u){right}..{down}(0.1u,-0.175u){down}..{left}',/, &
          '    (0u,-0.275u)--(0u,-0.425u){right}..{up}',/, &
          '    (0.25u,-0.175u){up}..{left}(0u,0.075u){left}..{up}',/, &
          '    (0u,0.075u){left}..{up}(-0.1u,0.175u){up}..{right}',/, &
          '    (0u,0.275u)--cycle;',/, &
          'fill A withcolor 0.9Dandelion;',/, &
          'draw A withpen pencircle scaled 0.7;')
