...
10         continue
           ...
           inA = ( 2.0d0 * p(1) / 3.0d0 ) ** 4 + ( 2.0d0 * p(2) ) ** 4 .le. 1.0d0
           if ( .not. inA ) then
              go to 10
           end if
           x(2*i-1) = p(1)
...
subroutine drawsol(n,x)
  ...
  write(filename,"(A12,I0,A3)")'rect-rounded',nballs,'.mp'
  ...
  do j = 1,npy
     ...
     do i = 1,npx
        ...
        fit = abs( ( 2.0d0 * px / 3.0d0 ) ** 4 + ( 2.0d0 * py ) ** 4 - 1.0d0 )
  ...
  ! Right side, y from 0.5 to 0.01 (negative step)
  npy = 200
  pyini = 0.01d0
  pyend = 0.5d0
  do j = 1,npy
  ...
  ! Right side, y from -0.01 to -0.5 (negative step)
  npy = 200
  pyini = - 0.5d0
  pyend = - 0.01d0
  do j = 1,npy
...
10 format('prologues := 3;',/, &
          ...
          'beginfig(',i3,');',/, &
          'u = ',f20.10,' cm;')
20 format('path rectrounded;',/, &
          'rectrounded := ');
21 format('     (',f20.10,'u,',f20.10,'u)--')
22 format('     cycle;',/, &
          'fill rectrounded withcolor 0.9Dandelion;',/, &
          'draw rectrounded withpen pencircle scaled 0.7;')
